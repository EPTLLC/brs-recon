...
class BRSLogger:
    ...
    def scan_complete(self, target: str, scan_type: str, duration: float, results_count: int = 0):
        ...
        self.console.print(text)
        self.logger.info(f"SCAN_COMPLETE: {scan_type} on {target} - {duration:.2f}s - {results_count} results")
    
    def scan_error(self, target: str, scan_type: str, error: str):
        """Log scan error"""
        text = Text(f"[FAILED] {scan_type} scan failed on {target}: {error}", style="bold red")
        self.console.print(text)
    ...
    def progress(self, message: str, current: int, total: int):
        ...
        text = Text(f"[PROGRESS] {message} ({current}/{total} - {percentage:.1f}%)", style="cyan")
        self.console.print(text)
    ...
    def result_saved(self, filename: str, result_type: str):
        ...
        text = Text(f"[SAVED] {result_type} results saved to {filename}", style="bold cyan")
        self.console.print(text)
