...
class BaseModule(ABC):
    ...
    def _end_scan(self, target: str, results: Dict[str, Any], error: Optional[str] = None) -> ScanResult:
    ...
    def _count_results(self, results: Dict[str, Any]) -> int:
        ...
        for key, value in results.items():
            if key in ["live_hosts", "open_ports", "subdomains", "services", "vulnerabilities"]:
...
class WebModule(BaseModule):
    ...
    def __init__(self, name: str, config: Optional[ScanConfig] = None):
        super().__init__(name, config)
        self.user_agent = "BRS-RECON/0.0.1 (Network Reconnaissance Tool)"
