...
from ..core.base import WebModule, ScanConfig
from ..core.utils import validate_domain, run_command, check_tool_availability
...
class DomainRecon(WebModule):
    ...
    def _resolver_ns(self) -> Optional[str]:
        ...
        try:
            return (getattr(getattr(self.config, "network", None), "dns_servers", None) or [None])[0]
        except Exception:
            return None
    ...
    def scan(self, target: str, **kwargs) -> Dict[str, Any]:
        ...
        if scan_type == "basic":
            ...
            results.update(self._comprehensive_recon(domain, include_subdomains, threads))
        else:
    ...
    def _comprehensive_recon(self, domain: str, include_subdomains: bool, threads: int) -> Dict[str, Any]:
    ...
    def _get_domain_info(self, domain: str, dns_records: Dict[str, List[str]]) -> Dict[str, Any]:
    ...
    def _attempt_zone_transfer(self, domain: str) -> Dict[str, Any]:
        ...
        name_servers = [x.strip().rstrip(".") for x in ns_res["stdout"].splitlines() if x.strip()]
        ...
        for ns in name_servers:
            try:
                ...
                if not res["success"] or not res["stdout"]:
                    continue
                s = res["stdout"]
                if "Transfer failed" in s or "refused" in s.lower() or "timed out" in s.lower():
                    continue
                if "XFR size" in s or "\tIN\t" in s:
                    out["possible"] = True
                    out["records"] = [l for l in s.splitlines() if l.strip()]
                    break
            except Exception as e:
    ...
    def _get_mx_details(self, domain: str) -> List[Dict[str, Any]]:
        ...
        for line in res["stdout"].splitlines():
            ...
            if aaaa["success"] and aaaa["stdout"].strip():
                ips.extend([x.strip() for x in aaaa["stdout"].splitlines() if x.strip()])
