...
class BRSRecon:
    ...
    def run_network_discovery(self, target: str, method: str = "ping_sweep", threads: int = 50):
        ...
        if result.status == "completed":
            ...
            self.logger.success(f"Network discovery completed - {len(live_hosts)} hosts found")
    ...
    def run_port_scan(self, target: str, ports: str = "common", scan_type: str = "tcp", 
                      threads: int = 100, service_detection: bool = False):
        """Run port scanning"""
        ...
        if result.status == "completed":
            ...
            self.logger.success(f"Port scan completed - {len(open_ports)} open ports found")
    ...
    def run_domain_recon(self, domain: str, scan_type: str = "basic", 
                        subdomains: bool = True, threads: int = 20):
        ...
        if result.status == "completed":
            ...
            self.logger.success(f"Domain reconnaissance completed")
...
def create_parser():
    ...
    network_parser.add_argument("--method", choices=["ping_sweep", "arp_scan", "nmap_discovery", "comprehensive"],
                               default="ping_sweep", help="Discovery method")
    network_parser.add_argument("--threads", type=int, default=50, help="Number of threads")
    network_parser.add_argument("--max-hosts", type=int, default=4096, help="Maximum hosts to scan")
    ...
    ports_parser.add_argument("--ports", default="common", 
                             help="Ports to scan (common, top100, top1000, all, or custom range)")
    ports_parser.add_argument("--scan-type", choices=["tcp", "udp", "syn", "comprehensive"],
                             default="tcp", help="Scan type")
    ports_parser.add_argument("--threads", type=int, default=100, help="Number of threads")
    ports_parser.add_argument("--service-detection", action="store_true", 
                             help="Enable service detection")
    ports_parser.add_argument("--max-ports", type=int, default=65535, help="Maximum ports to scan")
    ports_parser.add_argument("--insecure", action="store_true", 
    ...
    domain_parser.add_argument("--scan-type", choices=["basic", "comprehensive"],
                              default="basic", help="Scan type")
    domain_parser.add_argument("--no-subdomains", action="store_true",
                              help="Disable subdomain enumeration")
    domain_parser.add_argument("--threads", type=int, default=20, help="Number of threads")
    ...
    vuln_parser.add_argument("--scan-type", choices=["basic", "comprehensive", "web_only"],
                            default="basic", help="Scan type")
    vuln_parser.add_argument("--no-web", action="store_true", help="Disable web vulnerability scan")
    vuln_parser.add_argument("--no-ssl", action="store_true", help="Disable SSL vulnerability scan")
    vuln_parser.add_argument("--aggressive", action="store_true", help="Enable aggressive scanning")
    
    # System information command
    system_parser = subparsers.add_parser("system", help="System information")
    system_parser.add_argument("--target", default="localhost", help="Target system (default: localhost)")
    system_parser.add_argument("--scan-type", choices=["basic", "full"], default="basic", help="Scan type")
    system_parser.add_argument("--processes", action="store_true", help="Include process list")
    system_parser.add_argument("--no-network", action="store_true", help="Disable network info")
    system_parser.add_argument("--no-hardware", action="store_true", help="Disable hardware info")
    
    # Export command
    export_parser = subparsers.add_parser("export", help="Export results in multiple formats")
    export_parser.add_argument("scan_file", help="Path to scan result JSON file")
    export_parser.add_argument("--formats", nargs="+", choices=["json", "html", "sarif", "xml", "csv"],
                              default=["html"], help="Export formats")
...
def main():
    ...
    try:
        if args.command == "network":
            ...
            if scan_result:
                exported_files = results_manager.export_multi_format(scan_result, args.formats)
