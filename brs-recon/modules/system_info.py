...
from ..core.base import SystemModule, ScanConfig
from ..core.utils import run_command, format_bytes
...
class SystemInfo(SystemModule):
    ...
    def scan(self, target: str = "localhost", **kwargs) -> Dict[str, Any]:
        ...
        if scan_type == "basic":
            ...
            results.update(self._full_system_scan(include_processes, include_network, include_hardware))
        else:
            raise ValueError(f"Unknown scan type: {scan_type}")
        
        return results
    
    def _basic_system_scan(self, include_network: bool, include_hardware: bool) -> Dict[str, Any]:
        ...
        if platform.system() == "Linux" and SystemUtils.has_tool("systemctl"):
            systemctl_result = run_command(
                ["systemctl", "list-units", "--type=service", "--state=running", "--no-legend", "--no-pager", "--plain", "--all", "--full"],
                timeout=30
            )
            if systemctl_result["success"]:
                services = SystemUtils.parse_systemctl_output(systemctl_result["stdout"])
        ...
        results["scan_summary"] = {
            ...
            "network_interfaces": len(results.get("network_info", {}).get("interfaces", [])),
    ...
    def _full_system_scan(self, include_processes: bool, include_network: bool, include_hardware: bool) -> Dict[str, Any]:
