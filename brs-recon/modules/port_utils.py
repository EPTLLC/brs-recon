...
class PortUtils:
    ...
    def get_top_ports(count: int) -> List[int]:
        ...
        top = [80,443,22,21,25,23,53,110,139,445,135,143,993,995,3306,3389,5432,6379,27017,8080,8443,8000,8008,8081,111,2049,5900,5901,465,587,514,389,636,1521,1110,9100,10000,8888,123,161,162]
        # Dedup and sort
    ...
    def parse_port_specification(ports: str, logger) -> List[int]:
        ...
        if ports == "common":
            ...
            try:
                ...
                logger.warning(f"Invalid port specification: {ports}, using common ports")
                return PortUtils.get_common_ports()
    
    @staticmethod
    def tcp_connect(host: str, port: int, timeout: float = 3.0) -> Tuple[str, Optional[str]]:
        """Tri-state TCP connect with IPv6 support"""
        try:
            for fam, socktype, proto, _, sa in socket.getaddrinfo(host, port, type=socket.SOCK_STREAM):
            ...
            if port in (443, 8443, 9443):
                ...
                with socket.create_connection((host, port), 3) as raw:
                    with ctx.wrap_socket(raw, server_hostname=host) as ss:
                        ...
                        info["service"] = "HTTPS"
                        info["tls_subject"] = dict(x[0] for x in cert.get("subject", [])).get("commonName", "")
                        info["tls_issuer"] = dict(x[0] for x in cert.get("issuer", [])).get("commonName", "")
                return info
