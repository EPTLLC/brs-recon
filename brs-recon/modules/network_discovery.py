...
from ..core.utils import validate_network, validate_ip, run_command
...
class NetworkDiscovery(NetworkModule):
    ...
    def validate_requirements(self) -> bool:
        ...
        return status.get("nmap", False)  # не блокируемся на отсутствии ping, если есть nmap
    ...
    def scan(self, target: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            results["discovered_hosts"] = sorted(discovered, key=lambda x: ipaddress.ip_address(x))
        except:
    ...
    def _ping_sweep(self, target: str, threads: int) -> Dict[str, Any]:
        ...
        if hosts and self._check_network_tools(["fping"]).get("fping", False):
            ...
            if result["success"]:
                live_hosts = [l.strip() for l in result["stdout"].splitlines() if l.strip()]
            else:
        ...
        return {
            ...
            "scan_summary": {
                ...
                "success_rate": f"{(len(live_hosts)/len(hosts)*100):.1f}%" if hosts else "0%"
            }
        }
    ...
    def _nmap_discovery(self, target: str) -> Dict[str, Any]:
        ...
        cmd = ["nmap", "-sn", "-PE", "-PP", "-PS80,443", "-PA80,443", "-oG", "-", target]
        result = run_command(cmd, timeout=180)
        
        hosts = NetworkUtils.parse_nmap_grepable(result["stdout"]) if result["success"] else []
    ...
    def _comprehensive_discovery(self, target: str, threads: int) -> Dict[str, Any]:
        ...
        results["method_results"]["ping_sweep"] = ping_results["method_results"]["ping_sweep"]
        all_hosts.update(ping_results["live_hosts"])
        
        # Run ARP scan
        arp_results = self._arp_scan(target)
        results["method_results"]["arp_scan"] = arp_results["method_results"]["arp_scan"]
        all_hosts.update(arp_results["live_hosts"])
        
        # Run nmap discovery
        nmap_results = self._nmap_discovery(target)
        results["method_results"]["nmap_discovery"] = nmap_results["method_results"]["nmap_discovery"]
