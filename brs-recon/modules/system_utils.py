...
class SystemUtils:
    ...
    def get_filtered_disk_usage() -> List[Dict[str, Any]]:
        ...
        disk_usage = []
        skip_fstypes = {
            "tmpfs", "devtmpfs", "proc", "sysfs", "cgroup2", "overlay", 
            "squashfs", "autofs", "nsfs", "tracefs", "debugfs", "fusectl"
        }
    ...
    def get_connected_devices() -> List[Dict[str, Any]]:
        ...
        if platform.system() == "Linux":
            if SystemUtils.has_tool("lsusb"):
                ...
                if result["success"]:
                    devices += [{"type": "usb", "info": line.strip()} for line in result["stdout"].splitlines() if line.strip()]
            
            if SystemUtils.has_tool("lspci"):
                result = run_command(["lspci"], timeout=10)
                if result["success"]:
                    devices += [{"type": "pci", "info": line.strip()} for line in result["stdout"].splitlines()[:20] if line.strip()]
